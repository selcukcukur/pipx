...
impl Pipe<Vec<WorkItem>> for NormalizePayload {
    fn handle(
        &self,
        mut passable: Vec<WorkItem>,
        next: Next<'_, Vec<WorkItem>>,
...
impl Pipe<Vec<WorkItem>> for CalculateChecksum {
    fn handle(
        &self,
        mut passable: Vec<WorkItem>,
        next: Next<'_, Vec<WorkItem>>,
...
fn bench_full_pipeline_stress(c: &mut Criterion) {
    ...
    let pipes: Vec<PipelineStep<Vec<WorkItem>>> = vec![
        ...
        Arc::new(CalculateChecksum),
    ];

    group.bench_function("1000_items_pipeline", |b| {
        b.iter(|| {
